...
Each user-facing page is its own HTML file: `index.html` (hero + countdown + claim + hub cards), `luoghi.html` (venues + maps + transfer connector), `tavoli.html` (the seating-plan page: SVG sala + tavoli, alphabetical cantina list, detail card, and a full `?admin` editor; see `CONTEXT.md` for *Tavolo* / *Cantina*), `galleria.html` (list of guest spaces), `spazio.html?nick=X` (one guest's space + upload/lightbox), `giochi.html` (the **live host-paced quiz**; `?admin` = Sposi authoring + conducting — ADR-0005). Every page:
...
saveSettings(adminPin, s)  -> void        (couple only)
```
